...
use subxt::{self, Client, ClientBuilder, DefaultConfig, PairSigner, PolkadotExtrinsicParams};
use tokio::{
	self,
	time::{sleep, Duration},
};
...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let treasury_account = api.constants().fluent_fee().treasury_account().unwrap();
