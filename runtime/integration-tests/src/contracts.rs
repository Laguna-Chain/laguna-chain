...
fn test_ink_basic() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let rs = <Runtime as ContractsApi<Block, AccountId, Balance, BlockNumber, Hash>>::call(
				ALICE,
...
fn test_sol_basic() {
	ExtBuilder::default()
		...
		.execute_with(|| {
			...
			let blob =
				std::fs::read("../integration-tests/contracts-data/solidity/basic/dist/Basic.wasm")
					.expect("cound not find wasm blob");
